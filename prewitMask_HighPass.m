...
for p=2:r+1
    for q=2:c+1
        gx(p-1,q-1)=-j(p-1,q-1)-j(p-1,q)-j(p-1,q+1)+j(p+1,q-1)+j(p+1,q)+j(p+1,q+1);
        gy(p-1,q-1)=-j(p-1,q-1)-j(p,q-1)-j(p+1,q-1)+j(p-1,q+1)+j(p,q+1)+j(p+1,q+1);
...
title('Prewitt Sharpened Edges');
subplot(2,2,3), imshow(uint8(gx),[]);
title('Prewitt Horizontal Sharpened Edges');
subplot(2,2,4), imshow(uint8(gy),[]);
title('Prewitt Vertical Sharpened Edges');
